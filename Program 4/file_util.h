...
#define MAXBUFF 200
typedef struct tokn{
	char str[MAX_CHAR];  //the actual token
...
int getInFile(FILE **input); 			//reads user input checks if  input file exists and opens it
int getOutFile( FILE **output, FILE **listing, FILE **tmp); //gets user input  checks if output file exists, or opens a file input.OUT
...
int IF(FILE *listing, token tok[], int i, int tokperline[]);			////////// Functions of the Parser to determine if the semantic rules were upheld
...
void printListing(int tokperline[]);								   //prints the listing file
void gen_infix( token tok[], int i, int tokperline[]);				   //generates the temporary values for expressions
void appendFiles();										              //appends the tmp file to the output file
